...
public class Agents
{
    ...
    public interface Translator {
        @UserMessage("Translate this text to French: {{story}}")
        @Agent("Translates the draft (optional extra)")
        String translate(@V("story") String story);
    }
    ...
    public interface FactChecker {
        ...
        String check(@V("story") String story);
    }
    ...
    public interface DecisionProposer {
        ...
        @Agent("Proposes a decision for a human to confirm")
        String propose(@V("request") String request);
    }

    public static class WordCounter {
        @Agent(description = "Counts the words in the story", outputKey = "wordCount")
        public int count(@V("story") String story) {
            return story == null ||
                    story.trim().isEmpty() ? 0 : story.trim().split("\\s+").length;
        }
    }

    public interface DeclWriter {
        @UserMessage("Write a six sentence blog intro about {{topic}}.")
        @Agent(description = "Writes the first draft", outputKey = "story")
        String write(@V("topic") String topic);
    }

    public interface DeclEditor {
        @UserMessage("Polish this text so it flows well, keep the meaning: {{story}}")
        @Agent(description = "Polishes the writing", outputKey = "story")
        String polish(@V("story") String story);
    }

    public interface BlogCreator {
        @SequenceAgent(outputKey = "story", subAgents = {DeclWriter.class, DeclEditor.class})
